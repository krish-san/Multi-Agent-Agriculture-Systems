...
class ParallelForkNode:
    ...
    def split_task(self, main_task: Dict[str, Any], num_chunks: int = None) -> List[Dict[str, Any]]:
        try:
            if num_chunks is None:
                num_chunks = self.max_concurrent_tasks
            
            task_type = main_task.get('type', 'generic')
            task_data = main_task.get('data', {})
            
            sub_tasks = []
            
            if task_type == 'data_processing':
                # Split data processing tasks by data chunks
                data_items = task_data.get('items', [])
                chunk_size = max(1, len(data_items) // num_chunks)
                
                for i in range(0, len(data_items), chunk_size):
                    chunk = data_items[i:i + chunk_size]
                    sub_task = {
                        'id': f"{main_task.get('id', 'task')}_{i//chunk_size + 1}",
                        'type': task_type,
                        'data': {**task_data, 'items': chunk},
                        'chunk_index': i // chunk_size,
                        'parent_task_id': main_task.get('id')
                    }
                    sub_tasks.append(sub_task)
            
            elif task_type == 'text_analysis':
                # Split text analysis by text segments
                ...
                if text_content:
                    ...
                    for i in range(0, len(words), words_per_chunk):
                        ...
                        sub_task = {
                            'id': f"{main_task.get('id', 'task')}_{i//words_per_chunk + 1}",
                            'type': task_type,
                            'data': {**task_data, 'text': chunk_text},
                ...
                # Generic task splitting - create parallel instances
                for i in range(num_chunks):
                    sub_task = {
                        'id': f"{main_task.get('id', 'task')}_{i + 1}",
                        'type': task_type,
                        'data': task_data.copy(),
                        'chunk_index': i,
                        'parent_task_id': main_task.get('id')
            ...
            logger.info(f"Split task {main_task.get('id')} into {len(sub_tasks)} sub-tasks")
            return sub_tasks
            
        except Exception as e:
            logger.error(f"Failed to split task: {str(e)}")
            return [main_task]  # Return original task if splitting fails
    
    def distribute_work(self, sub_tasks: List[Dict[str, Any]], 
                       available_workers: List[str] = None) -> Dict[str, List[Dict[str, Any]]]:
        try:
            if not available_workers:
                available_workers = [f"worker_{i}" for i in range(self.max_concurrent_tasks)]
            
            # Distribute tasks round-robin among workers
            ...
            logger.info(f"Distributed {len(sub_tasks)} tasks among {len(available_workers)} workers")
            return worker_assignments
            
        except Exception as e:
            logger.error(f"Failed to distribute work: {str(e)}")
            return {}
...
class ParallelWorkerNode:
    ...
    def execute_parallel(self, task: Dict[str, Any], context: Dict[str, Any] = None) -> Dict[str, Any]:
        try:
            if context is None:
                context = {}
            ...
            # Get task type and determine execution method
            task_type = task.get('type', 'generic')
            task_data = task.get('data', {})
            
            if task_type == 'data_processing':
                result = self._execute_data_processing(task_data)
            elif task_type == 'text_analysis':
                result = self._execute_text_analysis(task_data)
            elif task_type == 'api_interaction':
                result = self._execute_api_interaction(task_data)
            else:
            ...
            logger.error(f"Parallel execution failed for task {task.get('id')}: {str(e)}")
            return {
                'task_id': task.get('id'),
                'status': 'failed',
                'error': str(e),
    ...
    def process_chunk(self, data_chunk: List[Any], processing_func: Callable = None) -> List[Any]:
        try:
            ...
            for item in data_chunk:
                try:
                    processed_item = processing_func(item)
    ...
    def _execute_data_processing(self, task_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _execute_text_analysis(self, task_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _execute_api_interaction(self, task_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            'method': method,
            'status': 'success',
            'mock_response': {'status': 200, 'data': 'mock_data'}
        }
...
class ParallelAggregatorNode:
    ...
    def aggregate_results(self, completed_results: List[Dict[str, Any]], 
                         failed_results: List[Dict[str, Any]] = None) -> Dict[str, Any]:
        try:
            if failed_results is None:
                failed_results = []
            
            total_tasks = len(completed_results) + len(failed_results)
            success_rate = len(completed_results) / total_tasks if total_tasks > 0 else 0
            
            # Aggregate data based on result type
            aggregated_data = self._combine_results_data(completed_results)
    ...
    def combine_outputs(self, outputs: List[Any], combination_strategy: str = 'merge') -> Any:
        try:
            if not outputs:
                return {}
            ...
            if combination_strategy == 'merge':
            ...
            elif combination_strategy == 'concatenate':
                ...
                if all(isinstance(output, list) for output in outputs):
                    ...
                    return combined
                elif all(isinstance(output, str) for output in outputs):
                    return ' '.join(outputs)
            
            elif combination_strategy == 'sum':
                # Sum numeric values
                if all(isinstance(output, (int, float)) for output in outputs):
                    return sum(outputs)
            
            elif combination_strategy == 'collect':
                # Simply collect all outputs in a list
                return list(outputs)
            
            # Default: return all outputs as a list
            return outputs
            
        except Exception as e:
            logger.error(f"Failed to combine outputs: {str(e)}")
            return outputs
    
    def _combine_results_data(self, completed_results: List[Dict[str, Any]]) -> Dict[str, Any]:
        combined_data = {}
        ...
        for result in completed_results:
            ...
            # Collect item counts for data processing tasks
            if 'processed_count' in result_data:
                item_counts.append(result_data['processed_count'])
            
            # Collect text analysis metrics
            if 'word_count' in result_data:
                if 'total_words' not in combined_data:
                    combined_data['total_words'] = 0
                combined_data['total_words'] += result_data['word_count']
            
            if 'character_count' in result_data:
                if 'total_characters' not in combined_data:
                    combined_data['total_characters'] = 0
                combined_data['total_characters'] += result_data['character_count']
        
        # Add aggregated metrics
        combined_data['total_execution_time'] = total_execution_time
        if item_counts:
            combined_data['total_items_processed'] = sum(item_counts)
